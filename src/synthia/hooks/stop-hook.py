    ...
    if remote_mode:
        ...
        try:
            ...
            if any(indicator in lower_msg for indicator in plan_indicators):
                if re.search(r"\d\.\s+\w", full_message) or re.search(
                    r"(?:first|then|next|finally)", lower_msg
                ):
                    is_plan = True
        ...
        try:
            ...
            tts = TextToSpeech(
                use_local=True,
                local_voice=config.get(
                    "local_tts_voice", "~/.local/share/piper-voices/en_US-lessac-high.onnx"
                ),
            )
